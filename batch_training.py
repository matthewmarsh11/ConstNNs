...
def main():
    ...
    mlp = MLP(
        config=model_config,
        input_dim=X_tensor_unconst.shape[1],
        output_dim=y_tensor.shape[1]
    )
    ...
    mlp, mlp_history, mlp_avg_loss = mlp_trainer.train(X_train_unconst, y_train, X_test_unconst, y_test, X_val_unconst, y_val, criterion)
    ec_nn, ec_history, ec_avg_loss = ec_trainer.train(X_train, y_train, X_test, y_test, X_val, y_val, criterion)
    # pinn, pinn_history, pinn_avg_loss = pinn_trainer.train(X_train_unconst, y_train, X_test_unconst, y_test, X_val_unconst, y_val, criterion)
